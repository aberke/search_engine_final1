...
static PyObject *searchio_difference_normsq(PyObject *self, PyObject *args);
...
static PyMethodDef SearchioMethods[] = {
    {"difference_normsq", &searchio_difference_normsq, METH_VARARGS, "helper for testing -- finds the norm of the difference of two vectors"},
...
/****************** ADDING C IMPLEMENTATION OF **********
 TODO: 
    ...
    def difference_normsq(vec1, vec2):
        ...
        return norm
*************************************/
/** added by alex but she needs to move it **/
static PyObject *searchio_difference_normsq(PyObject *self, PyObject *args){
    PyObject *vec1 = NULL;
    PyObject *vec2 = NULL;
...
#         2) matrix in sparse row dictionary form: {-1:(1/N), -2:(alpha/N), i:{j: P_i_j where i,j docIDs}} <-- -1 maps to entries of zero-rows, -2 maps to entries of zero-entries of non-zero rows
...
def vector_matrix_multiply(vector, matrix, keys):
    ...
    for j in keys:
        for i in keys:
            row_i = matrix[i] # might be empty dictionary
            m_ij = matrix[-1]
            if row_i:
                if j in row_i:
                    m_ij = row_i[j]
                    ...
                    m_ij = matrix[-2]
            result[j] += vector[i]*m_ij
    return result
*************************************/
/** added by alex but she needs to move it **/
// static PyObject *searchio_vector_matrix_multiply(PyObject *self, PyObject *args){
//     PyObject *vector = NULL;
//     PyObject *matrix = NULL;
//     PyObject *keys = NULL;
//     if (!PyArg_ParseTuple(args, "OOO", &vector, &matrix, &keys))
//         return NULL; 

//     /* create a result vector dictionary as well as array*/
//     PyObject *result = PyDict_New();
//     float result_array[num_keys];
// }
